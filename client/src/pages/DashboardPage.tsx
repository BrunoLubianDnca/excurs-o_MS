...
function BoardingPassHero({ trip, bundle, locale, onOpen, onEdit, onCopy, onArchive, onDelete }: {
  ...
  const passengerCount = trip.guest_count ?? 0
  const organizerCount = Math.max(1, buddyCount - passengerCount)
  const buddyLabel = passengerCount > 0
    ? (organizerCount === 1
      ? t('dashboard.hero.passengersOrganizerOne', { passengers: passengerCount })
      : t('dashboard.hero.passengersOrganizersMany', { passengers: passengerCount, organizers: organizerCount }))
  ...
  const passCells = (
    <>
      <div className="pass-cell buddies">
        ...
        <div className="date-month">{buddyLabel}</div>
      </div>
    ...
    >
      ...
      <div className="trip-body">
        ...
        <div className="trip-meta" style={{ gridTemplateColumns: 'repeat(3, 1fr)' }}>
          ...
          <div><span className="n mono">{trip.guest_count ?? trip.shared_count ?? 0}</span><span className="k">{trip.guest_count ? t('dashboard.passengers') : (trip.shared_count === 1 ? t('dashboard.card.buddyOne') : t('dashboard.members'))}</span></div>
        </div>

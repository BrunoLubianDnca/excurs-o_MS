  ...
  return (
    <TabScroller>
      ...
      <div className="mt-2 rounded-2xl border border-[color:var(--m-rowbr)] bg-m-card p-[13px]">
        ...
        {settleOpen && (
          <>
            {flows.length === 0 && totals.outstandingCount > 0 ? (
              <div className="py-[14px] text-center">
                <span className="mx-auto flex h-10 w-10 items-center justify-center rounded-full" style={{ background: 'rgba(232,161,58,.14)', color: STATUS_COLOR.pending }}>
                  <AlertCircle size={19} strokeWidth={2.5} />
                </span>
                <div className="mt-[7px] text-[0.84375rem] font-bold text-m-ink">{t('costs.assignmentPendingTitle')}</div>
                <div className="mx-auto mt-[2px] max-w-[270px] font-geist text-[0.65625rem] leading-relaxed text-m-faint">{t('costs.assignmentPendingBody')}</div>
              </div>
            ) : flows.length === 0 ? (
              <div className="py-[14px] text-center">

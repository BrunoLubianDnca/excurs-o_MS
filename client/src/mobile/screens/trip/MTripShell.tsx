...
import MSimpleTripOverview from './MSimpleTripOverview'
...
export default function MTripShell({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-[color:var(--m-bg)] bg-[image:var(--m-scr)] text-m-ink">
        ...
        <div className="absolute inset-0">
          ...
          {view === 'plan' && mode !== 'browse' && (
            <div className="absolute inset-0 z-10 bg-[color:var(--m-bg)] bg-[image:var(--m-scr)]">
              {mode === 'go'
                ? <MSimpleTripOverview planner={planner} shell={shell} />
      ...
      {days.length > 0 && !(trTab === 'plan' && mode === 'go' && view === 'plan') && (
      ...
      <nav className="absolute left-4 right-4 z-40 flex h-[68px] items-center justify-around rounded-[34px] border border-[color:var(--m-gbr)] bg-[color:var(--m-glass)] px-[7px] shadow-[0_16px_44px_-14px_rgba(0,0,0,.35)] backdrop-blur-[30px] backdrop-saturate-[1.8] bottom-[calc(env(safe-area-inset-bottom,0px)+10px)]">
        {dockTabs.map(({ id, icon: Icon }) => {
          ...
          return (
            <button
              ...
              className={`flex h-[50px] min-w-0 flex-1 flex-col items-center justify-center gap-[2px] rounded-[22px] ${
                active ? 'bg-m-act text-m-actfg' : 'text-m-muted'
              }`}
            >
              <Icon size={18} strokeWidth={2} />
              <span className="max-w-full truncate px-1 font-geist text-[0.5rem] font-bold leading-none">{t(`mobileTrip.dock.${id}`)}</span>
            </button>
          )
        })}
        <button
          ...
          className="flex h-[50px] min-w-0 flex-1 flex-col items-center justify-center gap-[2px] rounded-[22px] text-m-muted"
        >
          <MoreHorizontal size={18} strokeWidth={2.2} />
          <span className="font-geist text-[0.5rem] font-bold leading-none">{t('mobileTrip.dock.more')}</span>
        </button>
      </nav>

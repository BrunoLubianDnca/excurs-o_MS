...
function MSpotlightCard({ trip, t, onOpen, actions }: {
  ...
  const passengers = trip.guest_count ?? 0
  const organizers = Math.max(1, people - passengers)
  const peopleLabel = passengers > 0
    ? (organizers === 1
      ? t('dashboard.hero.passengersOrganizerOne', { passengers })
      : t('dashboard.hero.passengersOrganizersMany', { passengers, organizers }))
    : (people === 1 ? t('dashboard.hero.travelerOne', { count: people }) : t('dashboard.hero.travelerMany', { count: people }))
    ...
    >
      ...
      <div className="absolute bottom-[10px] left-[10px] right-[10px] rounded-[18px] border border-white/[.16] bg-[rgba(14,14,17,.52)] p-[12px_14px] text-white backdrop-blur-[22px] backdrop-saturate-[1.6]">{/* theme-lint-disable — fixed dark glass on the cover photo */}
        ...
        <div className={`flex gap-[6px] ${ongoing ? '' : 'mt-[9px]'}`}>
          ...
          <SpotlightPill icon={<Users size={11} strokeWidth={2.2} />} label={peopleLabel} />
        </div>
      </div>
    </div>
  )
    ...
    >
      ...
      <div className="p-[14px_16px_16px]">
        ...
        <div className="flex text-center">
          ...
          <ListStat value={trip.guest_count ?? trip.shared_count ?? 0} label={trip.guest_count ? t('dashboard.passengers') : (trip.shared_count === 1 ? t('dashboard.card.buddyOne') : t('dashboard.members'))} />
        </div>
        <MTripBadges items={pluginBadges} />
      </div>
    </div>
